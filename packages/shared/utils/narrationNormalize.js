...
function isSingleLetter(core) {
  return /^[A-Za-z]$/.test(core);
}

function parseDigitToken(core) {
  if (!core) return null;
...
export function stripDoubleFullstops(text) {
  return String(text || '').replace(/(^|[^.])\.\.(?!\.)/g, '$1.');
}

export function normalizeNarration(rawText) {
  ...
  const applyPhrase = (start, count, displayCore, kind = 'math') => {
  ...
  const applyCombinedDisplay = (start, end, displayCore, kind = 'math') => {
    const lead = tokens[start]?.leading || '';
    const trail = tokens[end]?.trailing || '';
    entries[start].displayCore = displayCore;
    entries[start].displayLeading = lead;
  ...
  while (i < tokens.length) {
    ...
    if (t?.lower === 'equals' || t?.lower === 'equal') {
      ...
      const canMultiply =
        isSingleLetter(tokens[i - 1]?.core || '') || FORMULA_CONTEXT.has(prev);
      if (
        canMultiply &&
  ...
  const tokenMap = entries.map((entry) => {
    ...
    let displaySpan = lastDisplaySpan;
    if (displayToken) {
...
export function mapWordTimingsToDisplay({ tokenMap, ttsText, displayText, ttsWordTimings }) {
  ...
  let tokenIdx = 0;
  const findDisplaySpan = (wordSpan) => {
    while (
      tokenIdx < tokenMap.length &&
      tokenMap[tokenIdx].ttsSpan[1] <= wordSpan.start
    ) {
      tokenIdx += 1;
    }
    ...
    if (
      wordSpan.start >= entry.ttsSpan[0] &&
      wordSpan.start <= entry.ttsSpan[1]
    ) {
  ...
  return ttsWordTimings.map((timing, idx) => {
    ...
    const text =
      displaySpan && displaySpan[1] > displaySpan[0]
        ...
        : timing.text || timing.w || span.text;

    if (typeof timing.start === 'number') {
      return { ...timing, text };
    }
    if (typeof timing.t === 'number') {
