...
export type Vendor = { id: ID; name: string; slug: string; status: VendorStatus; cuisine: string; rating: number; ratingCount?: number; prepMinutes: number; deliveryFee: number; minimumOrder: number; imageUrl?: string; acceptingOrders?: boolean; deliveryAvailable?: boolean; deliveryAvailabilityMessage?: string | null };
export type MenuItem = { id: ID; vendorId: ID; name: string; description: string; price: number; customerPrice?: number; commissionAmount?: number; commissionRateBps?: number; vatAmount?: number; taxRateBps?: number; taxCategory?: 'standard' | 'zero_rated' | 'exempt'; category: string; popular?: boolean; available: boolean; imageUrl?: string; sectionId?: ID; unitLabel?: string; barcode?: string; sortOrder?: number };
