...
function discountPctForCredits(credits) {
  if (credits < 10) return 0.0; // keep Starter (5) and 10- credits at full price
  if (credits >= 200) return 0.12; // 12%
  if (credits >= 100) return 0.1; // 10%
...
(async () => {
  ...
  try {
    ...
    console.log(
      'Inserting USD & KES packages (Starter 5→$1, tiered discounts 10+ credits)…',
    );
