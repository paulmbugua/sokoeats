...
async function paystackInitTransaction({
  ...
  const r = await fetch(`${PAYSTACK_BASE}/transaction/initialize`, {
    ...
    body: JSON.stringify({
      ...
      channels:
        Array.isArray(channels) && channels.length ? channels : ['card'],
...
async function paystackVerify(reference) {
  ...
  const r = await fetch(
    `${PAYSTACK_BASE}/transaction/verify/${encodeURIComponent(ref)}`,
    {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${PAYSTACK_SECRET_KEY}`,
...
async function activateOrgSubscriptionTx(client, payRow) {
  ...
  if (!Number.isFinite(seats) || seats <= 0) {
    throw new Error(
      `Missing/invalid seats mapping for tier "${tier}" in ORG_SEATS`,
    );
  }
  ...
  if (String(payRow.cycle) === 'monthly')
  ...
  await client.query(`UPDATE organizations SET updated_at=NOW() WHERE id=$1`, [
    payRow.org_id,
  ]);
...
/* ------------------------------------------------------------------ */
// ✅ UPDATED: initOrgSubscription with idempotency key support (both MPESA + PAYSTACK)
export async function initOrgSubscription(req, res) {
 ...
 const idemKey =
  ...
  try {
    if (method === 'MPESA') {
      ({ amount_cents } = resolvePrice(tier, cycle, 'KES')); // KES minor
      intentCurrency = 'KES';
    } else {
      ({ amount_cents } = resolvePrice(tier, cycle, 'USD')); // USD cents
      intentCurrency = 'USD';
    }
  } catch (e) {
    return res.status(400).json({
      message: 'Invalid plan selection',
      error: e?.message || 'bad-price',
    });
  }
  ...
  // ✅ NEW: reuse prior pending payment for same org+tier+cycle+provider+idemKey
  if (idemKey) {
    const existing = await pool.query(
      `SELECT *
        ...
        LIMIT 1`,
      [orgId, tier, cycle, method, idemKey],
    );

    if (existing.rowCount) {
      const pay = existing.rows[0];

      // Return a shape matching the method branch so FE can continue
      if (method === 'MPESA') {
        const meta =
          typeof pay.meta === 'string'
            ...
            : pay.meta || {};
        const expectedKesMinor = Number.isFinite(Number(meta?.expectedKesMinor))
          ? Number(meta.expectedKesMinor)
          : null;
        const expectedKesInt = Number.isFinite(Number(meta?.expectedKesInt))
          ? Number(meta.expectedKesInt)
          : null;
        ...
        return res.json({
          ...
          method: 'MPESA',
          reuse: true,
          charge: { currency: 'KES', expectedKesMinor, expectedKesInt },
      ...
      const meta =
        typeof pay.meta === 'string'
          ...
          : pay.meta || {};
      const expectedKesMinor = Number.isFinite(Number(meta?.chargeAmountMinor))
      ...
      return res.json({
        ...
        authorizationUrl: meta?.authorization_url || undefined, // may not exist in meta; FE can re-init if missing
        reference: pay.provider_order_id || null,
        idemKey,
      });
    }
  }
  ...
  [orgId, tier, cycle, intentCurrency, amount_cents, method, idemKey]
    ...
    logPgErr(
      '[orgBilling][init] INSERT org_subscription_payments failed',
      err,
      {
        orgId,
        tier,
  ...
  try {
    // ───────────────────────── MPESA (KES ONLY) ─────────────────────────
    if (method === 'MPESA') {
      const kesMinor = Number(amount_cents); // KES minor
  ...
  [
    ...
    idemKey,
      ...
      try {
        stk = await stkPushOrgSubscription({
          phone,
          amount: amountKesInt,
          accountReference: `ORG:${orgId}:${tier}:${cycle}`.slice(0, 12),
      ...
      if (!checkoutId) {
        ...
        return res
          .status(502)
          .json({ message: 'Invalid response from M-Pesa' });
      }
    ...
    // ─────────────────────── PAYSTACK (unchanged + idemKey stored) ───────────────────────
    ...
    if (!email) {
      ...
      return res.status(400).json({
        message:
          'Your account email is missing. Please update your profile email.',
      });
    }
    ...
    try {
      ...
      logPgErr('[orgBilling][init] failed to mark payment failed', e2, {
        paymentId: payment?.id,
      });
    }
...
export async function confirmOrgSubscription(req, res) {
  ...
  try {
    ...
    try {
      meta =
        typeof pay.meta === 'string' ? JSON.parse(pay.meta) : pay.meta || {};
    } catch {
      meta = {};
    }
    ...
    if (provider === 'MPESA') {
      ...
      if (!isSuccess) {
        if (resultCode != null && resultCode !== 0) {
          await client.query(
            ...
            [
              pay.id,
              `mpesa_failed_result_code_${resultCode}`,
              jsonbPatch({
                failReason: 'mpesa_result_failed',
      ...
      const expectedMinor = Number.isFinite(Number(meta?.expectedKesMinor))
        ...
        : String(pay.currency || '').toUpperCase() === 'KES' &&
            Number.isFinite(Number(pay.amount_cents))
      ...
      if (!Number.isFinite(expectedMinor) || expectedMinor <= 0) {
        ...
        return res.status(400).json({
          ok: false,
          status: 'failed',
          message: 'missing-expected-amount',
        });
      }

      const paidMinorFromMeta = Number.isFinite(Number(meta?.paidKesMinor))
        ? Number(meta.paidKesMinor)
        : null;
      ...
      const finalRef =
        mpesaReceipt ||
        (provider_reference ? String(provider_reference).trim() : null);
    ...
    const reference = String(
      provider_reference || pay.provider_order_id || '',
    ).trim();
    if (!reference) {
      await client.query('ROLLBACK');
    ...
    if (gotCurrency !== 'KES') {
      ...
      await client.query('COMMIT');
      return res.status(400).json({
        message: `currency-mismatch (expected KES, got ${gotCurrency})`,
      });
    }
    ...
    if (
      ...
      return res
        .status(400)
        .json({ message: 'amount-mismatch', expectedMinor, paidMinor });
    }
