...
export async function listTopCoursesService({
  ...
  scoredAll.sort(
    (a, b) =>
      b._score - a._score || b.rating - a.rating || b.reviews - a.reviews,
  );
...
export async function generateOutlineService({
  ...
  const preset = await resolveCourseSize({
    courseId,
    bodyCourseSize: courseSize,
    programTrack,
  });
  ...
  dlog('outline', 'computed plan', {
    totalLessons,
    targetMinutesTotal: target,
    fromTrack: lessonsForTrack(programTrack),
    fromSize: totalLessonsOf(preset),
  });
  ...
  const wantTokens = Math.min(
    12000,
    Math.max(1200, perItemTokens * totalLessons + 300),
  );
  ...
  function maxTokensForSlice(count) {
    const proportional = Math.floor(
      wantTokens * (count / Math.max(1, totalLessons)),
    );
    ...
    return Math.min(
      6000,
      Math.max(elasticFloor, Math.min(proportional, heuristic)),
    );
  }
  ...
  async function genSlice(start, count, overrideMaxTokens) {
    ...
    const localMax =
      Number.isFinite(overrideMaxTokens) && overrideMaxTokens > 0
  ...
  try {
    // chunk if large
    const CHUNK =
      totalLessons > 40 ? 20 : totalLessons > 30 ? 30 : totalLessons;
    ...
    for (let i = 0; i < totalLessons; i += CHUNK) {
      ...
      const capForThisSlice = Math.min(
        maxTokensForSlice(take),
      ...
      if (capForThisSlice > 0) {
        try {
          ...
          console.warn(
            `[${LOG_NS}:outline] slice ${i}-${i + take - 1} failed; using fallback`,
            e?.message,
          );
        }
      } else {
        dlog('outline', 'budget exhausted; using fallback', {
          start: i,
          count: take,
          budgetRemaining,
        });
    ...
    console.warn(`[${LOG_NS}:outline] error`, {
      kind: c.kind,
      status: c.status,
      msg: err?.message,
    });
    ...
    if (c.kind === 'auth') {
      return {
        status: 401,
        data: { error: 'OpenAI API key invalid or unauthorized' },
        headers: {},
      };
    }
    if (c.kind === 'timeout') {
    ...
    return {
      ...
      data: {
        outline: fb,
        notice: fallbackNotice('outline_repaired_or_fallback'),
      },
      headers: {
        'X-Degraded': 'true',
        'X-Computed-Lessons': String(totalLessons),
...
function ensureAnchorsForArtifacts(lesson, ssml) {
  const body = (
    ssml.match(/<prosody[^>]*>([\s\S]*?)<\/prosody>/i)?.[1] || ssml
  ).replace(/<[^>]+>/g, ' ');
...
function closeProsodyIfMissing(ssml) {
  ...
  if (opens > closes) {
    ...
    return ssml.replace(
      /<\/voice>\s*<\/speak>\s*$/i,
      `${'</prosody>'.repeat(need)}</voice></speak>`,
    );
  }
  return ssml;
}
...
export async function generateLessonSSMLService(
  ...
  const cq = await pool.query(`SELECT title FROM courses WHERE id = $1`, [
    courseId,
  ]);
  if (!cq.rowCount)
    return { status: 404, data: { error: 'COURSE_NOT_FOUND' }, headers: {} };
  ...
  const safeStart = Math.max(
    0,
    Math.min(Number(start) || 0, Math.max(0, outline.length - 1)),
  );
  ...
  const takeCount = Math.max(
    1,
    Math.min(wantCount, Math.max(1, outline.length - safeStart)),
  );
  ...
  const outlineHash = sha1(
    JSON.stringify({ slice: outlineSlice, start: safeStart }),
  );
  ...
  if (FAST_FIRST && isFirstClip) {
          ...
          generateLessonSSMLService(
            {
              ...
              voiceName,
              courseSize,
              count: Math.min(PREWARM, outline.length - 1),
              start: 1,
              programTrack,
            },
    ...
    return {
      status: 200,
      data: payload,
      headers: { 'X-Cache': 'MISS', 'X-Fast-First': '1' },
    };
  }
...
Write one self-contained lesson per section with a hook, goals, core concept, worked example, pitfall, a micro-check, and a recap.`,
    ...
    const lessons = [];
    const rawLessons = (Array.isArray(json?.lessons) ? json.lessons : []).slice(
      0,
      takeCount,
    );
    ...
    for (let i = 0; i < rawLessons.length; i++) {
      ...
      const id = `L${absoluteIdx + 1}`;
      const title = String(
        l?.title || outlineSlice[i]?.title || `Lesson ${absoluteIdx + 1}`,
      );
      const goals = Array.isArray(l?.goals) ? l.goals.slice(0, 6) : [];
      const estSeconds = Number(
        l?.estSeconds ||
          Math.round((preset.estAudioMinSec + preset.estAudioMaxSec) / 2),
      );
...
const renderSnippet = (sn) => {
  ...
  return `${header}\`\`\`${lang}\n${sn.code || ''}\n\`\`\`\n`;
    ...
    if (!lessons.length) {
      console.warn(
        `[${LOG_NS}:lesson] AI returned empty lessons; retrying plain SSML`,
      );
      try {
        ...
        return {
          ...
          data: {
            ...payload,
            notice: { degraded: true, reason: 'json_parse_failed_plain_ssml' },
          },
          headers: {
            'X-Cache': 'MISS',
            'X-Degraded': 'true',
        ...
        console.warn(
          `[${LOG_NS}:lesson] plain SSML retry failed; falling back to scaffold`,
        );
        const pack = scaffoldFromOutline();
        const produced = pack.lessons?.length ?? 0;
        const hasMore = safeStart + produced < outline.length;
        ...
        return {
          ...
          data: {
            ...payload,
            notice: { degraded: true, reason: 'ai_empty_lessons' },
          },
          headers: {},
        };
      }
...
${bodies.join('\n')}
    ...
    log('log', 'lesson', 'success', {
      ...
      joinedBytes: joinedSsml.length,
    });

    const artifactTotals = lessons.reduce(
      (acc, L) => {
    ...
    try {
      ...
      if (okToPrewarm && nextStart != null) {
        const preCount = Math.min(
          PREWARM,
          Math.max(0, outline.length - nextStart),
        );
        setImmediate(() => {
          generateLessonSSMLService(
            {
    ...
    const c = classifyOpenAIError(err);
    console.warn(`[${LOG_NS}:lesson] error`, {
      kind: c.kind,
      status: c.status,
      msg: err?.message,
    });
    ...
    if (c.kind === 'timeout' || c.kind === 'network') {
      return {
        status: 503,
        data: { error: c.message || 'temporary_error' },
        headers: { 'Retry-After': String(c.retryAfterSec || 5) },
      };
    }
    if (c.kind === 'auth') {
      return {
  ...
  const push = (q) => {
    ...
    if (t === 'mcq') {
      ...
      if (choices.length !== 4) {
        choices = choices
          .slice(0, 4)
          .concat(['Option A', 'Option B', 'Option C', 'Option D'])
          .slice(0, 4);
      }
      let answerIndex = Number.isFinite(Number(q.answerIndex))
  ...
  if (!['mcq', 'short'].includes(qt)) {
    return {
      ...
      data: {
        ...
        message: "quizType must be 'mcq' or 'short' (explicit).",
      },
      headers: {},
    };
  }
  ...
  return { items, timerSec };
    ...
    for (let i = 0; i < n; i += CHUNK) {
      ...
      try {
        ...
        console.warn(
          `[${LOG_NS}:quiz] slice ${i}-${i + take - 1} failed; continuing`,
          e?.message,
        );
      }
    }
...
const keptFromAI = Math.min(uniqueAi, normalized.length);
    ...
    return {
      ...
      data: {
        quiz,
        ...(degraded
          ? { notice: fallbackNotice('quiz_repaired_or_fallback') }
          : {}),
      },
      headers: {
    ...
    const c = classifyOpenAIError(err);
    console.warn(`[${LOG_NS}:quiz] error`, {
      kind: c.kind,
      status: c.status,
      msg: err?.message,
    });
    ...
    return {
      ...
      data: {
        ...
        notice: fallbackNotice(
          c.kind === 'bad_request' ? 'bad_request' : 'server_error',
        ),
      },
...
export async function generateCoursePackageService({
  courseId,
  level = 'beginner',
  targetMinutes,
  ...
  // Outline
  let outline,
    outlineResp = await generateOutlineService({
      courseId,
      title: courseTitle,
      level,
  ...
  if (outlineResp.status === 200) outline = outlineResp.data.outline;
    ...
    outline = makeFallbackOutline(courseTitle).slice(0, totalLessonsOf(preset));
  ...
  for (let i = 0; i < outline.length; i++) {
    try {
      const r = await generateLessonSSMLService({
        courseId,
        outline,
        voiceName,
        courseSize,
      ...
      if (L) {
        ...
        const tmp = await generateLessonSSMLService({
          courseId,
          outline: [outline[i]],
          voiceName,
          courseSize,
      ...
      const kp = Array.isArray(outline[i]?.keyPoints)
        ? outline[i].keyPoints.slice(0, 4)
        : [];
      const fallback = {
        id: `L${i + 1}`,
        title,
  ...
  const bodies = ssmlParts.map((s, i) => {
    ...
    return i === 0
      ? body
      : `<p><break time="${pace.sectionBreakMs}ms"/></p>\n${body}`;
  });
  const joinedSsml = `
<speak version="1.0" xml:lang="en-US" xmlns:mstts="http://www.w3.org/2001/mstts">
  <voice name="${voiceName}">
...
${bodies.join('\n')}
  ...
  const anyDegraded =
    [outlineResp.status, quizResp.status].some((s) => s && s !== 200) ||
    anyDegradedLesson;
