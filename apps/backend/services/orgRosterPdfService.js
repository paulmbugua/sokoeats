...
/* Cloudinary fetching with optional signed retry (same pattern as ExamPdf) */
...
async function tryLoadImageBuffer(idOrUrl, { w, h, trim = false, exact = false, dpr = 2 } = {}) {
  if (!idOrUrl) return null;

  const looksLikePublicId = typeof idOrUrl === 'string' && !idOrUrl.includes('://');
  const looksLikeCloudinaryUrl = typeof idOrUrl === 'string' && idOrUrl.includes('res.cloudinary.com');

  if (looksLikePublicId || looksLikeCloudinaryUrl) {
    const buf = await fetchCloudinaryAsPngBuffer(idOrUrl, { w, h, trim, exact, dpr });
    if (buf) return buf;
  }
