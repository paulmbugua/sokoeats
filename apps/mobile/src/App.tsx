...
function RiderProfileScreen({ data, onBack }: { data: GenericPayload; onBack: () => void }) {
  const hasRatings = data.ratingBreakdown.some((row: GenericPayload) => Number(row.value) > 0);
  return <View style={styles.riderShell}><RiderScreenHeader title="Rider Profile" status={data.rider.status} onBack={onBack} /><ScrollView contentContainerStyle={styles.riderContent}><View style={styles.profileHero}>{data.rider.avatarUrl ? <Image source={{ uri: data.rider.avatarUrl }} style={styles.profileAvatar} /> : <View style={[styles.profileAvatar, styles.profileAvatarFallback]}><Text style={styles.profileAvatarInitials}>{data.rider.initials}</Text></View>}<Text style={styles.checkoutTitle}>{data.rider.name}</Text><Text style={styles.restaurantMeta}>{data.rider.since}</Text><Text style={styles.restaurantMeta}>{data.rider.vehicle}</Text><Text style={styles.balanceText}>{data.rider.rating}</Text><Text style={styles.upperLabel}>{data.rider.reviews}</Text></View><View style={styles.signedInCard}><Text style={styles.vendorName}>Registered details</Text><View style={styles.priceLine}><Text style={styles.priceLabel}>Email</Text><Text style={styles.priceValue}>{data.rider.email}</Text></View><View style={styles.priceLine}><Text style={styles.priceLabel}>Phone</Text><Text style={styles.priceValue}>{data.rider.phone || 'Not added'}</Text></View><View style={styles.priceLine}><Text style={styles.priceLabel}>City</Text><Text style={styles.priceValue}>{data.rider.city || 'Not added'}</Text></View></View><View style={styles.riderStats}>{data.stats.map((stat: GenericPayload) => <View style={styles.riderStat} key={stat.label}><Text style={styles.riderStatValue}>{stat.value}</Text><Text style={styles.upperLabel}>{stat.label}</Text></View>)}</View><View style={styles.deliveryRequestCard}><Text style={styles.vendorName}>Ratings breakdown</Text>{hasRatings ? data.ratingBreakdown.map((row: GenericPayload) => <View style={styles.ratingLine} key={row.stars}><Text>{row.stars}</Text><View style={styles.deliveryProgress}><View style={[styles.deliveryProgressFill, { width: String(row.value) + '%' as any }]} /></View></View>) : <Text style={styles.smsBody}>Customer ratings will appear here after completed deliveries receive feedback.</Text>}</View>{data.qualities.length > 0 && <View style={styles.riderQuickGrid}>{data.qualities.map((item: GenericPayload) => <View style={styles.riderQuickButton} key={item.label}><Text style={styles.riderQuickTitle}>{item.label}</Text><Text style={styles.riderQuickText}>{item.value}</Text></View>)}</View>}<Text style={styles.checkoutSectionTitle}>Achievements</Text>{data.achievements.length > 0 ? <View style={styles.tabsRow}>{data.achievements.map((item: string) => <Text style={styles.tabPillActive} key={item}>{item}</Text>)}</View> : <Text style={styles.smsBody}>Achievements unlock from verified delivery activity.</Text>}<Text style={styles.checkoutSectionTitle}>Recent feedback</Text>{data.feedback.length > 0 ? data.feedback.map((entry: GenericPayload) => <View style={styles.uploadCard} key={entry.customer + entry.age}><Text style={styles.avatarInitial}>{entry.initials}</Text><View style={{ flex: 1 }}><Text style={styles.vendorName}>{entry.customer}</Text><Text style={styles.restaurantMeta}>{entry.age}</Text><Text style={styles.smsBody}>{entry.body}</Text></View></View>) : <Text style={styles.smsBody}>No customer feedback yet.</Text>}</ScrollView><BottomNav active="Account" variant="rider" /><SourceLedger /></View>;
}
...
const styles = StyleSheet.create({
  ...
  profileAvatarFallback: { alignItems: 'center', justifyContent: 'center', backgroundColor: colors.primaryContainer },
  profileAvatarInitials: { color: colors.onPrimaryContainer, fontSize: 32, lineHeight: 38, fontWeight: '900' },
