...
const SelectField: React.FC<SelectFieldProps> = ({
  ...
  const defaultPlaceholder = resolvedScheme === 'dark' ? '#94A3B8' : '#64748B'; // slate-400 (dark) / slate-500 (light)
  const defaultSelected = resolvedScheme === 'dark' ? '#E5E7EB' : '#0F172A'; // gray-200 / slate-900
  ...
  return (
    <>
      <View style={tw`mb-4`}>
        {label ? (
          <Text style={tw`text-sm font-semibold text-[#0d141c] dark:text-white mb-2`}>{label}</Text>
        ) : null}
        ...
        >
          ...
          <FontAwesome name={open ? 'chevron-up' : 'chevron-down'} size={14} color={iconColor} />
        </TouchableOpacity>

        {error ? (
          <Text style={tw`mt-1 text-[11px] text-red-600 dark:text-red-400`}>{error}</Text>
        ) : null}
      </View>
      ...
      <Modal transparent visible={open} animationType="fade" onRequestClose={() => setOpen(false)}>
        <View style={tw`flex-1 bg-black/50 justify-center px-6`}>
          <View
            style={tw`rounded-2xl bg-white dark:bg-[#0f1821] p-4 max-h-[80%] border border-slate-200 dark:border-white/10`}
          >
            ...
            >
              <Text style={tw`text-sm text-slate-700 dark:text-slate-100`}>Close</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
