...
const isDocish = (c: any): boolean => {
  ...
  if (/(book|textbook|pdf|ebook|document|doc|docs|article|page|html|note|notes|handout|worksheet|guide|summary)/.test(kind))
    return true;
...
const CardFadeIn: React.FC<React.PropsWithChildren<{ index?: number }>> = ({ children, index = 0 }) => {
...
const reloadOer = useCallback(async () => {
  ...
  const [r1, r2] = await Promise.all([
    fetch(`${base}/api/oer/collections?kind=doc&limit=48&raster=1`).then((r) => (r.ok ? r.json() : [])),
    fetch(`${base}/api/oer/collections?kind=video&limit=48&raster=1`).then((r) => (r.ok ? r.json() : [])),
  ]);
...
const onRefresh = useCallback(async () => {
  ...
  type MixedVideoItem = { kind: 'recorded'; data: RecordedVideo } | { kind: 'oerCollection'; data: OerCollection };
  ...
  const featuredNormalCourses = useMemo(() => (featuredCourses as Course[]).slice(0, VISIBLE_LIMIT * 2), [featuredCourses]);
                ...
                >
                  <CardFadeIn index={idx}>
                    <View style={tw`rounded-2xl p-3 bg-white dark:bg-[#0f1821] border border-[#cedbe8] dark:border-white/10`}>
                      <Image source={{ uri: t.image }} style={tw`w-16 h-16 rounded-full self-center`} resizeMode="cover" />
              ...
              <View style={tw`mt-3 flex-row flex-wrap -mx-1`}>
                {featuredCoursesDisplay.slice(0, VISIBLE_LIMIT).map((c: any, idx) => {
                  ...
                  return (
                    ...
                    >
                      <CardFadeIn index={idx}>
                        <View style={tw`rounded-2xl p-4 bg-white dark:bg-[#0f1821] border border-[#cedbe8] dark:border-white/10`}>
            ...
            <SectionReveal scrollY={scrollY} offset={160}>
              <View style={tw`mt-3`}>
                {freeVideoCollections.slice(0, VISIBLE_LIMIT).map((col, idx) => {
                  const thumb = pickThumb(col, backendUrl) || FALLBACK_CARD(col.title);
                  return (
                    <TouchableOpacity key={`col-${col.id}`} onPress={() => goCollection(String(col.id), 'video')} activeOpacity={0.9}>
                      <CardFadeIn index={idx}>
                        <View style={tw`mb-3 rounded-2xl p-4 bg-white dark:bg-[#0f1821] border border-[#cedbe8] dark:border-white/10`}>
                          <CardMedia src={thumb} title={col.title} />
                          <Text numberOfLines={1} style={tw`font-semibold text-[#0d141c] dark:text-white`}>
