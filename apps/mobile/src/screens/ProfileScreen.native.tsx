...
const looksLikeProfile = (prof: any): boolean => {
  ...
  if (prof.id != null || prof.user_id != null || prof.profile_id != null)
    return true;
...
const StudentProgressRow: React.FC<{
  ...
  const pct = useMemo(() => {
    ...
    if (loading) return 0;
    const items: CourseProgress[] = Array.isArray(progress)
      ? progress
      : [];
  ...
  return (
    ...
    >
      <View style={tw`flex-row items-center justify-between`}>
        <Text
          numberOfLines={1}
          style={tw`text-base font-medium text-[#0d141c] dark:text-white`}
        >
          {title}
        </Text>
        <Text
...
const ProfileScreen: React.FC = () => {
  ...
  const goSettingsManage = () =>
    navigation.navigate('SettingsManage');
  const goSettingsCreate = () =>
    navigation.navigate('SettingsCreate');
  const goMessages = () =>
  ...
  const goAccountEarnings = () =>
    navigation.navigate('Account', { tab: 'earnings' });
  const goAccountSessions = () =>
    navigation.navigate('Account', { tab: 'sessions' });
  ...
  useEffect(() => {
    ...
    const needRole =
      !ctxRole &&
      !(profile as ProfileLike | undefined)?.role;
  ...
  const [serverHasProfile, setServerHasProfile] = useState<
    boolean | null
  >(null);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const exists = await fetchHasProfile(
        backendUrl,
        token,
      );
      if (!cancelled) {
  ...
  const hasAnyProfile =
    hasCtxProfile || serverHasProfile === true;
  ...
  const resolvedRoleRaw = String(
    p.role || ctxRole || meRole || '',
  ).trim();
  ...
  const canSeeEarnings = useMemo(
    () => isTutor && hasPayoutSetup(p),
    [isTutor, p],
  );
  ...
  const [refreshingTokens, setRefreshingTokens] =
    useState(false);
  ...
  const handleChangeAvatar = useCallback(async () => {
    if (!backendUrl || !token) {
      Alert.alert(
        'Unavailable',
        'Missing backend configuration. Please try again later.',
      );
      return;
    }
    ...
    try {
      ...
      const remoteUrl = await uploadProfileImageNative(
        backendUrl,
        token,
        result.assets[0],
      );
  ...
  // ─────────────────────────────────────
  const { data: transactions = [] } = useAppQuery<
    Transaction[],
    Error
  >(
  ...
  const [earnErr, setEarnErr] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (status === 401)
          setEarnErr(
            'Please log in again to view earnings.',
          );
        else if (status === 403)
          setEarnErr(
            'Earnings are restricted for your account.',
          );
        else
          setEarnErr(
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Text
            style={tw`mb-3 text-sm font-semibold text-[#49739c] dark:text-white/70`}
          >
            Shortcuts
          </Text>
          ...
          <View style={tw`flex-row flex-wrap gap-2`}>
            ...
            >
              <Text
                style={tw`text-sm font-medium text-[#0d141c] dark:text-white`}
              >
                Home
              </Text>
            </Pressable>
        ...
        {isTutor &&
          ...
          !loadingProfile && (
            ...
            >
              <Text
                style={tw`font-semibold text-amber-900 dark:text-amber-200`}
              >
                No tutor profile found
              </Text>
              <Text
                style={tw`text-sm mt-0.5 text-amber-900/90 dark:text-amber-200/90`}
              >
                You’re signed in as a tutor. Create your profile
                so students can discover and book you.
              </Text>
              <View style={tw`mt-2`}>
                <Pressable
                  onPress={onEditOrCreateProfile}
        ...
        <Text
          style={tw`pt-4 pb-2 text-[20px] font-bold text-[#0d141c] dark:text-white`}
        >
          Personal information
        </Text>
        <View style={tw`gap-4`}>
          <View>
            <Text
              style={tw`pb-2 font-medium text-[#0d141c] dark:text-white`}
            >
              Full name
            </Text>
            <TextInput
        ...
        {/* Payments / Earnings */}
        <Text
          style={tw`pt-6 pb-2 text-[20px] font-bold text-[#0d141c] dark:text-white`}
        >
          {isTutor
            ? 'Tutor earnings'
            : 'Payment management'}
            ...
            >
              ...
              >
                {earnLoading ? (
                  ...
                  <Text
                    style={tw`text-sm text-[#49739c]`}
                  >
                    Set up your payout method to enable
                    earnings.{' '}
                    <Text
                      onPress={onEditOrCreateProfile}
                  ...
                  <>
                    ...
                    <Text
                      style={tw`text-3xl font-extrabold text-[#0d141c] dark:text-white`}
                    >
                      {fmtMoney(
                        earn.available,
                        earn.currency,
              ...
              {canSeeEarnings &&
                ...
                !earnErr && (
                  ...
                  >
                    <View
                      style={tw`flex-1 rounded-lg p-3 bg-[#e7edf4]/60 dark:bg-[#172534]`}
                    >
                      <Text
                        style={tw`text-[#49739c]`}
                      >
                        Total earned
                      </Text>
                      <Text
      ...
      <View style={tw`gap-3`}>
        ...
        {isStudent && (
          <View style={tw`pt-6`}>
            <Text
              style={tw`text-[20px] font-bold mb-3 text-[#0d141c] dark:text-white`}
            >
              Learning progress
            </Text>
            {enrLoading && (
                ...
                <View style={tw`gap-3`}>
                  {enrollments
                    ...
                    .map((e: Enrollment) => {
                      const courseId =
                        getCourseId(e);
                      const maybeTitle = (e as any)[
                        'title'
                      ];
                      const title =
                        typeof maybeTitle ===
                          'string' &&
                        maybeTitle.trim().length > 0
                          ? maybeTitle
                          : courseId
                          ? `Course #${courseId}`
                          : 'Course';
                      const fallbackPct = toNum(
                        (e as any).progress,
                        0,
                      );
                      return (
                  ...
                  {enrollments.length === 0 && (
                    ...
                    >
                      <Text
                        style={tw`text-base text-[#0d141c] dark:text-white`}
                      >
                        You have no enrollments yet.
                      </Text>
                      <Text
                        style={tw`text-sm text-[#49739c] mt-1`}
                      >
        ...
        {isTutor ? (
          <View style={tw`gap-3`}>
            ...
            >
              ...
              <Text
                style={tw`text-[#8b5e00] dark:text-amber-200/90 text-sm mt-1`}
              >
                Upload recorded classes & notes.
                Students purchase with tokens — you earn
                automatically.
              </Text>
              <Text
            ...
            >
              ...
              <Text
                style={tw`text-[#0b3a70] dark:text-blue-200/90 text-sm mt-1`}
              >
                Use our step-by-step builder to publish
                structured lessons, quizzes & certificates.
              </Text>
              <Text
        ...
        <View style={tw`gap-3`}>
          ...
          >
            ...
            >
              <View
                style={tw`w-10 h-10 rounded-lg bg-[#e7edf4] dark:bg-[#172534]`}
              />
              <Text
        ...
        <View style={tw`py-4`}>
          ...
          >
            ...
            >
              <Text
                style={tw`font-bold text-[#0d141c] dark:text-white`}
              >
                Log out
              </Text>
            </Pressable>
