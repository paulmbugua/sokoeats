...
export function PartnerPortal({ session, onSignOut }: { session: Session; onSignOut: () => void }) {
  ...
  const createItem = async () => {
    if (!item.name.trim() || !item.sectionTitle || Number(item.price) <= 0) return setMessage('Product name, category and a valid price are required.');
    setBusy(true); setMessage('Uploading and publishing product...');
    try {
      const imageUrl = await uploadImage();
      const result = await api<{ menu: PartnerMenu }>('/api/vendor/menu/items', { method: 'POST', body: JSON.stringify({ ...item, price: Number(item.price), imageUrl, available: true }) });
      setMenu(result.menu); setItem((current) => ({ ...current, name: '', description: '', price: '', unitLabel: '' })); setFile(null); setMessage('Product is visible to customers.');
  ...
  return <main className="partnerPortal">
    ...
    <section className="partnerWorkspace">
      ...
      <section><h2><PackagePlus/> Add product</h2><label>Category<select value={item.sectionTitle} onChange={(e) => setItem({ ...item, sectionTitle: e.target.value })}><option value="">Choose category</option>{menu?.sections.map((section) => <option key={section.id}>{section.title}</option>)}</select></label><label>Specific product name<input value={item.name} onChange={(e) => setItem({ ...item, name: e.target.value })} placeholder="Nyama Choma Platter"/></label><label>Description<textarea value={item.description} onChange={(e) => setItem({ ...item, description: e.target.value })} placeholder="Ingredients, size and key details"/></label><div className="partnerInputRow"><label>Price in KES<input type="number" min="1" value={item.price} onChange={(e) => setItem({ ...item, price: e.target.value })}/></label><label>Unit<input value={item.unitLabel} onChange={(e) => setItem({ ...item, unitLabel: e.target.value })} placeholder="plate, 1kg"/></label></div><label className="fileInput"><ImagePlus/><span>{file?.name || 'Choose product image'}</span><input type="file" accept="image/jpeg,image/png,image/webp,image/avif" onChange={(e) => setFile(e.target.files?.[0] || null)}/></label><button className="primary" disabled={busy} onClick={createItem}>{busy ? 'Working...' : 'Publish product'}</button></section></aside>
