...
function MerchantManager({ session, onSignOut }: { session: NonNullable<Session>; onSignOut: () => void }) {
  ...
  const saveCompliance = async () => {
    ...
    try {
      const result = await api<{ compliance: VendorCompliance }>('/api/vendor/compliance', { method: 'PUT', body: JSON.stringify({ ...complianceForm, commissionRateBps: 1500, commissionAgreementVersion: 'marketplace-v1' }) });
