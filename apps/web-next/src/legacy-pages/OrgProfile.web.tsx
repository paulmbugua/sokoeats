...
// apps/web-next/src/legacy-pages/OrgProfile.web.tsx
...
const OrgProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    if (!mounted || orgToken) return;
    const params = new URLSearchParams({
      returnTo: pathname || '/org/profile',
      reason: 'auth',
      message: 'Please sign in to continue',
    });
    router.replace(siteUrl(`/institutions/login?${params.toString()}`));
  }, [mounted, orgToken, pathname, router]);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6 bg-slate-50 dark:bg-darkBg">
        <div className={`${cardBase} w-full max-w-md p-6`}>
          ...
          <p className="mt-2 text-sm text-gray-600 dark:text-darkTextSecondary">Loading…</p>
        </div>
      </div>
    );
  }
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="text-sm text-[#49739c] dark:text-darkTextSecondary">
                  Attendance, fees &amp; balances, newsletters, and announcements for your
                  institution.
                </p>
              </div>
              <Link
