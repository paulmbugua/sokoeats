...
const OER_READER_ROUTE_BASE = siteUrl('/oer/collections');
...
const hrefForItem = (it: HomeCard) => {
  ...
  if (kind === 'video') return siteUrl(`/oer/collections/${id}`); // open playlist/video flow
  if (kind === 'text') return siteUrl(`/oer/${id}`); // open HTML reader

  // sensible defaults (keeps all OER in the OER readers)
  if (kind === 'pdf' || kind === 'book') return siteUrl(`/oer/${id}`);
  return siteUrl(`/oer/${id}`);
};

// OER readers
const getOerDocHref = (c: any) =>
  siteUrl(`/oer/${encodeURIComponent(String(c?.slug ?? c?.id ?? ''))}`);
const getOerCollectionHref = (c: any) =>
  siteUrl(`/oer/collections/${encodeURIComponent(String(c?.slug ?? c?.id ?? ''))}`);
...
const getHrefForItem = (c: any) => {
  ...
  // 1) OER only
  if (isOerLike(c)) {
    if (ckind.includes('video')) return siteUrl(`/oer/collections/${id}`);
    return siteUrl(`/oer/${id}`); // OER text/books to OER reader
  }

  // 2) Non-OER videos → recorded video/detail
  if (isVideoish(c)) return appUrl(`/class-vault/${id}`);

  // 3) Everything else (including non-OER “text/PDF-ish”) → normal course details
  return siteUrl(`/courses/${encodeURIComponent(String(c?.id ?? ''))}`);
};
        ...
        <div className="mx-auto w-full max-w-screen-2xl px-4 sm:px-6 lg:px-8 py-8 lg:py-10">
          ...
          >
            <motion.div className="flex items-center justify-between px-1" variants={fadeUp}>
              ...
              <Link href={siteUrl('/find-tutor')} className="text-primary hover:underline">
                See All Tutors
              </Link>
            </motion.div>
          ...
          >
            <motion.div className="flex items-center justify-between px-1" variants={fadeUp}>
              ...
              <Link
                href={siteUrl('/resources?tab=videos')}
            ...
            <div className="mt-4 grid grid-cols-1 gap-4 md:hidden">
              ...
              {featuredVideosMixed.slice(0, FEATURED_VIDEOS_LIMIT_MOBILE).map((item, idx) => {
                    ...
                    >
                      <div className="bg-white dark:bg-[#0f1821] rounded-xl overflow-hidden ring-1 ring-gray-200 dark:ring-darkCard hover:ring-primary transition">
                        ...
                        <div className="p-4">
                          ...
                          <div className="mt-3">
                            <Link
                              href={href || appUrl(`/class-vault/${v.id}`)}
                ...
                return (
                  ...
                  >
                    <Link
                      href={hrefForItem({ id: col.id, kind: 'video' })}
          ...
          >
            <motion.div className="flex items-center justify-between px-1" variants={fadeUp}>
              ...
              <Link href={siteUrl('/courses')} className="text-primary hover:underline">
                Browse All
              </Link>
            </motion.div>
          ...
          >
            <motion.div className="flex items-center justify-between px-1" variants={fadeUp}>
              ...
              <Link href={siteUrl('/courses?free=1')} className="text-primary hover:underline">
                Browse Free
              </Link>
            </motion.div>
